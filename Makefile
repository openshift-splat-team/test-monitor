...
ifeq ($(NO_DOCKER), 1)
	...
	DOCKER_CMD := $(ENGINE) run --env GO111MODULE=$(GO111MODULE) --env GOFLAGS=$(GOFLAGS) --rm -v "$(PWD)":/go/src/github.com/openshift-splat-team/test-monitor:Z  -w /go/src/github.com/openshift-splat-team/test-monitor $(BUILD_IMAGE)
	...
	# DOCKER_CMD := $(ENGINE) run --env GO111MODULE=$(GO111MODULE) --env GOFLAGS=$(GOFLAGS) --rm -v "$(PWD)":/go/src/github.com/openshift-splat-team/test-monitor:Z -w /go/src/github.com/openshift-splat-team/test-monitor registry.ci.openshift.org/ocp/builder:rhel-8-golang-1.19-openshift-4.11
